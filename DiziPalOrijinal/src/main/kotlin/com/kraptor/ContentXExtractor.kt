...
open class TurkeyPlayer : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        fixM3u?.contains("master.txt")?.let {
            ...
            Log.d("kraptor_DziPlOrjTrPlay", "normalized m3u » $fixM3u")
...
open class VidMoxy : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        Log.d("kraptor_DizipalOrj", "url = $url")
        val extRef   = referer ?: ""
        val videoReq = app.get(url, referer=extRef).text
//        Log.d("kraptor_DizipalOrj", "videoReq = $videoReq")
        val regex = Regex("""file\s*:\s*EE\.dd\("([^"]+)"""")
        val match = regex.find(videoReq)
        val encoded =  match?.groupValues?.get(1).toString()
        Log.d("kraptor_DizipalOrj", "encoded = $encoded")
        val decoded = decodeEE(encoded)
        Log.d("kraptor_DizipalOrj", "decoded = $decoded")
        ...
        altyRegex.findAll(videoReq).map { match ->
            ...
            val subLang = url
                .substringAfterLast("/")
                .substringBefore("_")
            Log.d("kraptor_DizipalOrj", "subLang = $subLang")
